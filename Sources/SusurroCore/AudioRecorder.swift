...
public final class AudioRecorder: @unchecked Sendable {
    ...
    // report a non-standard multichannel format (observed: a non-standard multichannel format).
    // Earlier attempts fed that whole multichannel buffer through AVAudioMixerNode's or
    ...
    // unrecognized/discrete multichannel layout collapses to silence, independent of
    ...
    // result is that something was asked to *mix down* the input channels to 1 using an
    // automatic channel matrix. a multichannel AVAudioFormat with no explicit
